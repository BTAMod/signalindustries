...
public class WakingCrusherPage
    ...
    public WakingCrusherPage(GuidebookSection section, ArrayList<RecipeEntryMachine> recipes) {
        super(section);
    ...
    protected void renderForeground(RenderEngine re, FontRenderer fr, int x, int y, int mouseX, int mouseY, float partialTicks) {
        drawStringCenteredNoShadow(fr, "Waking Crusher", x+width - 158 / 2, y+5, 0xFF808080);
        if(recipes.isEmpty()){
        ...
        for (int i = 1; i <= recipes.size(); i++) {
            ...
            if(recipe.getData().chance < 1){
                drawStringCentered(fr,(recipe.getData().chance*100)+"%",x + list.get(list.size()-1).xDisplayPosition - 19, y +  list.get(list.size()-1).yDisplayPosition + 4,0xFFFFFFFF);
            }
