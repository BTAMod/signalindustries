...
public class CentrifugeRecipeCategory implements IRecipeCategory<MachineRecipeTranslator> {
    ...
    public CentrifugeRecipeCategory() {
        ...
        this.icon = new DrawableIngredient<>(SIBlocks.reinforcedCentrifuge.getDefaultStack(), ItemStackIngredientRenderer.INSTANCE);
    ...
    public void drawRecipe(ITMBRuntime runtime, MachineRecipeTranslator recipe, IRecipeLayout layout, List<IIngredientList> ingredients, ILookupContext context) {
        ...
        ingredients.add(5, new IngredientList(ExtendedTypedIngredient.fluidStackIngredient(new FluidStack(SIFluids.SIGNALUM_ENERGY, (int) (data.cost * (data.ticks/200.0f))))));

        if (data.thisTierOnly) {
            runtime.getGuiHelper().getMinecraft().fontRenderer.drawStringWithShadow("Only at: "+data.tier.getTextColor() + data.tier.getRank() + TextFormatting.WHITE,24,(background.getHeight()),0xFFF0F0F0);
        } else {
            runtime.getGuiHelper().getMinecraft().fontRenderer.drawStringWithShadow("Minimum tier: "+data.tier.getTextColor()+data.tier.getRank() + TextFormatting.WHITE,24,(background.getHeight()),0xFFF0F0F0);
        }
        ...
        runtime.getGuiHelper().getMinecraft().fontRenderer.drawCenteredString(data.ticks+"t",x + 59, (background.getHeight() / 2) - 14,0xFFFFFFFF);
        runtime.getGuiHelper().getMinecraft().fontRenderer.drawCenteredString((data.chance*100)+"%",x + 59, (background.getHeight() / 2) + 14,0xFFFFFFFF);
    }

    @Override
    public IRecipeLayout getRecipeLayout() {
        return new RecipeLayoutBuilder()
                .addInputSlot(0, TMBFluidPlugin.FLUID_STACK).setPosition(x - 20, (background.getHeight() / 2) - 6).build()
                .addInputSlot(1, TMBFluidPlugin.FLUID_STACK).setPosition(x, (background.getHeight() / 2) + 16).build()
