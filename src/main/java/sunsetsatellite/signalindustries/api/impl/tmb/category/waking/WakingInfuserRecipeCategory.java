...
public class WakingInfuserRecipeCategory implements IRecipeCategory<MachineRecipeTranslator> {
    ...
    public WakingInfuserRecipeCategory() {
        this.background = new DrawableBlank(120, 80);
        this.icon = new DrawableIngredient<>(SIBlocks.wakingInfuser.getDefaultStack(), ItemStackIngredientRenderer.INSTANCE);
    ...
    public void drawRecipe(ITMBRuntime runtime, MachineRecipeTranslator recipe, IRecipeLayout layout, List<IIngredientList> ingredients, ILookupContext context) {
        ...
        ingredients.add(4, new IngredientList(ExtendedTypedIngredient.fluidStackIngredient(new FluidStack(SIFluids.SIGNALUM_ENERGY, (int) (data.cost * (data.ticks/200.0f))))));
